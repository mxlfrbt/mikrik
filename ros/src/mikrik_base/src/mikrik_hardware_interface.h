...
#ifndef MIKRIK_HARDWARE_INTERFACE_HPP_
#define MIKRIK_HARDWARE_INTERFACE_HPP_
...
class MikrikHardwareInterface : public hardware_interface::RobotHW {
public:
	MikrikHardwareInterface(ros::NodeHandle node, ros::NodeHandle private_node, double target_max_wheel_angular_speed);
...
MikrikHardwareInterface::MikrikHardwareInterface(ros::NodeHandle node, ros::NodeHandle private_node, double target_max_wheel_angular_speed)
	: _node(node)
	, _private_node(private_node)
	, _max_wheel_angular_speed(target_max_wheel_angular_speed) {
	registerControlInterfaces();

	_left_wheel_vel_pub = _node.advertise<std_msgs::Float64>("/mikrik/left_wheel/target_velocity", 1);
	_right_wheel_vel_pub = _node.advertise<std_msgs::Float64>("/mikrik/right_wheel/target_velocity", 1);
	_left_wheel_angle_sub = _node.subscribe("mikrik/left_wheel/angle", 1, &MikrikHardwareInterface::leftWheelAngleCallback, this);
	_right_wheel_angle_sub = _node.subscribe("mikrik/right_wheel/angle", 1, &MikrikHardwareInterface::rightWheelAngleCallback, this);
}

void MikrikHardwareInterface::writeCommandsToHardware() {
...
void MikrikHardwareInterface::updateJointsFromHardware(const ros::Duration& period) {
...
void MikrikHardwareInterface::registerControlInterfaces() {
...
void MikrikHardwareInterface::leftWheelAngleCallback(const std_msgs::Float64& msg) {
	_left_wheel_angle = msg.data;
}

void MikrikHardwareInterface::rightWheelAngleCallback(const std_msgs::Float64& msg) {
	_right_wheel_angle = msg.data;
}

void MikrikHardwareInterface::limitDifferentialSpeed(double& diff_speed_left_side, double& diff_speed_right_side) {
...
#endif // MIKRIK_HARDWARE_INTERFACE_HPP_
